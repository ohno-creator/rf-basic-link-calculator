...
| # | 内容 | 状態 |
|---|---|---|
| A1 | i3シェルv3＋本文色統一 → feature統合 | ✅ 統合・push済み（全ページのヘッダー階層・余白・prose幅が変化） |
| A2 | H2: 全6チャートにスナップ強調ドット＋破線ガイド | 🔄 CI視覚ループ走行中 → 完了次第revert→統合 |
| A3 | H7: 図版SVG/PNG保存ボタン（旗艦滝グラフ手本） | 🔄 CI視覚ループ走行中 → 同上 |

### W-B: 全ページv3量産 —「統一感」の本丸（W-A完了後すぐ・Claude実施）
0. **B0 逸脱監査**（🔄 並列監査実行中）: I1タイポ/I2余白/直書きhex/I4エレベーションの逸脱を全ページで件数化。
1. 監査結果をページ群3便に分けて量産: **便1=旗艦 → 便2=NCU → 便3=2ペイン群＋共有部品**。
2. 各便: trackブランチ→機能ゲート（tsc/vitest/lint/build/e2e）→CI視覚ループ→revert→統合。
3. 完了定義: design-base-v3 §4 の grep 監査**0違反**・全ゲート緑・視覚差分が意図どおり。

### W-C: 図版統一の残り（W-Bと並行可）
- H2ツールチップ/カーソル統一の全図版波及確認、H7保存ボタンの横展開（NCU断面・フレネル・定在波・NCU滝 等）。
