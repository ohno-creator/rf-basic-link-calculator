...
| H6 | **3D可視化の導入（新規）** | 価値が明確な2箇所から: (a) **NCU地下BOXの3Dシーン**（蓋材質・アンテナ位置・水分を3Dで直感表示、断面図と切替）、(b) **アンテナ3D放射パターン**（球面プロット、利得を色と半径で）。react-three-fiber＋drei を **dynamic import＋ページ内遅延**で導入し `output: export` と両立。**2Dフォールバック必須**（SSR時・WebGL不可時）。バンドル予算: 3D同梱ページのみ +150KB gzip 以内、他ページへの影響ゼロをCIで検証 | Claude設計・実装→Antigravity実機/性能検証 |
