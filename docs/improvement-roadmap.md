...
### W-C: 図版統一の残り — ✅ 完了（2026-07-06）
- SVG図版8ファイルの残hexを diagramPalette で同値置換（NCU断面・NCU滝・アンテナ ほか、R5bとして統合完了）。
- Metricロール本適用（値表示 text-lg font-bold 5箇所）・要判断の表セルslate-700。
- H7保存ボタンの横展開（NCU断面・フレネル・定在波・NCU滝 等、R5bとして統合完了）。

### W-D: ユーザー判断待ち — ✅ 完了（2026-07-06）
- **I5 相談CTA削減（6→2）**: 経営判断A案に基づき、ResultDetails＋ページ末ConsultationCtaの2箇所に集約完了、マージ済み。
- **H6改 NCU地下BOX 2.5Dシーン**: 2.5D等角投影の基本レイアウト（iso.ts）を適用完了。
