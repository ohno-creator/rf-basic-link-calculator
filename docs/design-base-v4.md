...
|---|---|---|
| **R1 タイポ基盤** | v4-1 フォント導入（Inter＋Noto Sans JP・woff2自己ホスト131スライス） | ✅ 実装済・CI視覚ループ中 |
| **R2 ホーム/信頼/CTA** | トラストストリップ／CTAブランド面（v4-2）／フッター強化／アイコンタイル2トーン＋ツール名タイポ是正／初心者カード角丸是正／ヒーロー計測方眼（v4-3） | 次便 |
| **R3 ツールページ主役化** | essenceLead「このツールでわかること」行／ResultBar主役化＋警告集約／Stat=Metric仕様（単位小さく淡色・ベースライン揃え）／Card 2.5D（v4-4）／パンくずカテゴリリンク化 | R2後 |
| **R4 図版レポート品質** | **エクスポート忠実度バグ修正**（SVG内TailwindクラスをdiagramText属性化＋getComputedStyleインライン化フォールバック）／滝グラフのマージン寸法ブラケット（v4-6）／ChartFrame＋凡例/軸タイトル定型／H7ボタン横展開／図版残hexのパレット置換（旧W-C吸収） | R3後 |
| **R5 磨き（低優先）** | ブランドロックアップ／sticky category filter／ホバー浮遊統一／2ペインToolTwoPane部品化／負号U+2212／text-2xs昇格 | 任意 |
