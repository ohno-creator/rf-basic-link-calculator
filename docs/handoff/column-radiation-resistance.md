...
|---|---|---|---|---|---|---|---|
| S1 | C. A. Balanis (2016) | book | N/A (ISBN: 978-1-118-64206-1) | Chapter 4, Sec 4.2 & 4.3 | *"Antenna Theory: Analysis and Design"* (4th Ed)。微小ダイポール（Infinitesimal Dipole）および短い線状ダイポール（Small Dipole）の放射電磁界、放射抵抗 of 導出。 | 2026-07 | ○ (書籍) |
...
| S3 | H. A. Wheeler (1975) | paper | [ieee.org](https://ieeexplore.ieee.org/document/1142106) | Vol. AP-23, pp. 462-469 | *“Small antennas”* (IEEE Trans. Antennas Propag.)。短いダイポールやモノポールアンテナの等価放射抵抗および実効効率の極限理論。 | 2026-07 | ✅確認済 |
...
3. **短縮モノポール（Small Monopole - 三角形電流分布、GND面無限大）**
   - **数値**: 
     $$R_{r,\text{small\_monopole}} = 40 \pi^2 \left( \frac{h}{\lambda} \right)^2 \approx 394.8 \left( \frac{h}{\lambda} \right)^2 \quad \text{[Ω]}$$
     ここで $h$ はモノポールアンテナの高さ（$h \ll \lambda/4$）。
   - **係数の注意**: 全長 $l=2h$ の短いダイポールに $20\pi^2(l/\lambda)^2$ を適用し、完全導体地板上のモノポール入力抵抗がその半分になることから、$10\pi^2(2h/\lambda)^2=40\pi^2(h/\lambda)^2$。$10\pi^2$ を使う場合の変数は $h$ ではなく鏡像を含む全長 $2h$。
