# 第6波 実行プラン（2026-07-12 改訂v7・3者分担の正本）
...
- **v7新規（ユーザー発注）**: **周波数の用途地図（非セルラー編）**——携帯網以外の周波数用途を世界9地域で見渡し、日本は共用・再編・**周波数再編アクションプラン（令和7年度版・2025-11-28公表・Claude確認済み）**・WG活動まで深掘りする新ツール＋用途カテゴリ別コラム9本（発注書: prompt-codex-spectrum-atlas.md）
- 進行中（Claude）: O-1/O-1bの先行レビューWorkflowをバックグラウンド実行中（term-lab 21語×4群＋OTAlib並列レビュー→敵対的検証。作業ツリーの現状スナップショットに対する読み取り専用レビュー）
...
| O-5 | F1改/F3の検収 | G-4待ち |
| O-6 | Antigravity代行検証（Web調査・出典裏取り・検算に限定） | 継続（A-1/A-3検算はクローズ済み。今回: アクションプラン令和7年度版の一次情報アンカー確認） |
| **O-7** | **【新規】周波数用途地図のデータ裏取り**: G-7納品後、日本エントリ全数＋各地域サンプルの一次情報照合（unverified-current→confirmed-current昇格判断） | G-7納品待ち |
...
| **G-7** | **【新規】周波数の用途地図（非セルラー編）**: 5モードUI（ものさし/用途別/日本詳細3タブ/世界比較/検索）＋SpectrumEntry型データ（日本20+・主要国8+・他5+エントリ）＋カタログ関数＋**用途カテゴリ別コラム9本（D1構造化）**。確定アンカー（令和7年度アクションプラン等）は発注書に記載済み・裏取り不能分はunverified-currentで正直に | prompt-codex-spectrum-atlas.md |
...
| A-7 | **周波数用途地図の実機受入＋残数裏取り**（5モード官能評価・O-7で未確定のエントリの全数裏取り・375px） | G-7＋O-7後 |
| 済 | ~~A-1~~ Band v3裏取り✅／~~A-3検算~~ 5点照合✅（いずれもクローズ） | — |

## 3. リリース順序（推奨・v7改訂）
**① term-lab（G-1）を最優先で確定**（実装済み資産を早くmainへ・消失リスクの報告書も同梱）→ ② OTAエキスパート（G-2・lib完成済みでUIのみ）→ ③ 効率変換（G-0）・半値角（G-0b）→ ④ **周波数用途地図（G-7→O-7）**——データ量が大きいため③までの小粒と並行着手も可 → ⑤ コラム移行10本単位（G-3+G-3fix）→ ⑥ F1改/F3（G-4→O-5）→ ⑦ E2第2段（G-6）。
