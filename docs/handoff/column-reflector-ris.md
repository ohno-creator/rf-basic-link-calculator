...
2. **遠方界（Far-field）におけるRIS/反射板アシストリンクのパスロス**
   - **数値**: 
     $$PL_{\text{far}} = \frac{P_t}{P_r} = \frac{(4\pi)^3 d_1^2 d_2^2}{G_t G_r \lambda^2 \cos(\theta_t) \cos(\theta_r) A^2}$$
     （または単位セル分割表現：素子数 $N$ に対して $N^2$ に反比例）
   - **条件**: 送信点〜反射板距離 $d_1$、反射板〜受信点距離 $d_2$ が共に遠方界限界（$d \ge \frac{2 D^2}{\lambda}$）を満たす状態。
   - **物理的意味**: 反射板が小さく「点波源の二次散乱体（受動リレー）」として振る舞うため、損失は距離の積 $d_1^2 d_2^2$ （4乗則）に依存して非常に大きくなります。
   - **出典**: [S1] Özdogan (2020) Eq (10) / [S2] Tang (2020)。
