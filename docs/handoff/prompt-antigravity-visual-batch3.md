...
## 追記2: H4/H5 も feature 済（`759631d`/`76c6dbc`）— 本検証に含めてください

- **H5 ナミゲートヒートマップ（viridis化・見どころ）**: 離散6色→**viridis連続配色**（power: -100〜-50dBm固定 / diff: 0〜+8dB固定・0未満=グレー維持）。凡例が**連続グラデバー＋min/max目盛り**に。配色が大きく変わるため before/after を必ず確認（対象: nami-gate-window の3モード）
- **H4 旗艦滝グラフv2**: バーホバーで内訳ツールチップ（ネイティブ<title>）・送信/受信バーに柔影・0dBm線ラベル・数値tabular化。クリックジャンプ等の既存機能は不変
- **要目視**: (a) viridisの紫〜黄がナミゲートのブランド文脈で違和感ないか、(b) セル数値ラベルの可読性（輝度ベース自動白/墨）、(c) 滝グラフの影が過剰でないか

## 既知の残り（次バッチ予定）
- H2 マイクロインタラクション（十字線・スナップ）／H6 3Dスパイク／H7 エクスポート
