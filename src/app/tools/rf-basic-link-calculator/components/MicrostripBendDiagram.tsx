...
export function MicrostripBendDiagram({ angleDeg, miterPercent, cutbackMm }: MicrostripBendDiagramProps) {
  ...
  return (
    <figure className="rounded-lg border border-slate-200 bg-slate-50 p-4">
      ...
      <svg viewBox="0 0 360 210" role="img" aria-label="マイクロストリップの曲げ。外側の角を斜めにカットしたマイター形状を示す上面図。" className="mt-2 w-full">
        {/* 線路（入射・出射） */}
        <polygon points={incoming} fill="rgba(180,83,9,0.18)" stroke="#b45309" strokeWidth="1" />
        <polygon points={outgoing} fill="rgba(180,83,9,0.18)" stroke="#b45309" strokeWidth="1" />
      ...
      <p className="mt-2 text-xs leading-relaxed text-slate-600">
        曲げの外側の角は、そのままだと余分な容量が付いて反射の原因になります。角を斜めにカット（マイター）すると整合が改善します。対角のカット長は約 {cutbackMm.toFixed(2)} mm
