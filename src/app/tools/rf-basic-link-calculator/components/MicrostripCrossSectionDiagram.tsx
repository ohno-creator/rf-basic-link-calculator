  ...
  return (
    <figure className="rounded-lg border border-slate-200 bg-slate-50 p-4">
      ...
      <svg viewBox="0 0 360 196" role="img" aria-label="マイクロストリップ線路の断面図。基板の上の導体とグラウンド面、誘電体を示す。" className="mt-2 w-full">
        {/* 誘電体（基板） */}
        <rect x={SUB_LEFT} y={subTop} width={SUB_W} height={round(GROUND_Y - subTop)} fill={`rgba(0,113,189,${dielectricAlpha})`} stroke="#cbd5e1" />
        {/* グラウンド面 */}
        <rect x={SUB_LEFT} y={GROUND_Y} width={SUB_W} height="8" fill="#b45309" />
        {/* フリンジング電界 */}
        ...
        {/* 導体（線路） */}
        <rect x={traceLeft} y={traceTop} width={round(wPx)} height={TRACE_H} rx="1.5" fill="#b45309" />

        {/* ラベル */}
        <text x={CENTER_X} y={round(traceTop - 6)} textAnchor="middle" fontSize="11" fontWeight="700" fill="#92400e">
          導体 W
        </text>
        <text x={CENTER_X} y={round((subTop + GROUND_Y) / 2 + 4)} textAnchor="middle" fontSize="11" fill="#0071BD">
          基板 εr {dielectricConstant}
        </text>
        <text x={CENTER_X} y={GROUND_Y + 20} textAnchor="middle" fontSize="11" fill="#92400e">
          グラウンド面（GND）
        </text>
        ...
        <g transform="translate(266, 36)">
          <text x="0" y="0" fontSize="11" fill="#64748b">特性インピーダンス</text>
          <text x="0" y="20" fontSize="18" fontWeight="700" fill="#0071BD">{impedanceOhms.toFixed(1)} Ω</text>
          <text x="0" y="52" fontSize="11" fill="#64748b">実効比誘電率 εeff</text>
          <text x="0" y="72" fontSize="18" fontWeight="700" fill="#0f172a">{effectiveDielectric.toFixed(2)}</text>
          <text x="0" y="104" fontSize="11" fill="#64748b">速度係数 VF</text>
          <text x="0" y="124" fontSize="18" fontWeight="700" fill="#0071BD">{velocityFactor.toFixed(3)}</text>
        </g>
      </svg>
