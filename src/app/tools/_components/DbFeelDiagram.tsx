...
import { diagramStroke, diagramText } from "@/lib/ui/diagramTheme";
  ...
  return (
    <figure className="rounded-lg border border-slate-200 bg-slate-50 p-4">
      ...
      <svg viewBox="0 0 560 116" role="img" aria-label="dBの目盛りと電力倍率の対応を示すものさし。等間隔のdBが2倍・10倍・100倍に対応する。" className="mt-2 w-full">
        {/* 軸 */}
        <line x1={X0} y1={AXIS_Y} x2={X1} y2={AXIS_Y} stroke="#94a3b8" strokeWidth={diagramStroke.main} />
        ...
        {anchors.map((anchor) => {
          ...
          return (
            <g key={anchor.db}>
              ...
              <text x={x} y={AXIS_Y + 22} textAnchor="middle" fontSize={diagramText.label.fontSize} fill="#0071BD">
                {anchor.ratio}
              </text>
            </g>
          );
        ...
        {showStack ? (
          <g>
            <line x1={stackX} y1={AXIS_Y - 14} x2={stackX} y2={AXIS_Y + 30} stroke="#64748b" strokeWidth={diagramStroke.main} strokeDasharray="4 3" />
        ...
        {/* 現在のdBマーカー */}
        <line x1={markerX} y1={AXIS_Y - 26} x2={markerX} y2={AXIS_Y + 30} stroke="#0071BD" strokeWidth={diagramStroke.emphasis} />
