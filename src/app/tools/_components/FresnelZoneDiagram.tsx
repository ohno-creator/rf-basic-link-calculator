...
const VERDICT_COLOR: Record<ObstacleAnalysis["verdict"], { fill: string; stroke: string; text: string }> = {
  clear: { fill: "rgba(16,185,129,0.25)", stroke: diagramPalette.successDeep, text: diagramPalette.successDeep },
  caution: { fill: "rgba(245,158,11,0.32)", stroke: diagramPalette.amberDeep, text: diagramPalette.amberDeep },
  blocked: { fill: "rgba(244,63,94,0.30)", stroke: diagramPalette.dangerDeep, text: diagramPalette.dangerDeep }
};
    ...
    <figure className="rounded-lg border border-slate-200 bg-slate-50 p-4">
      ...
      <DiagramExportButton filenameBase="fresnel-zone">
        <svg viewBox="0 0 580 248" role="img" aria-label="送受信アンテナの間に広がる第1フレネルゾーンと、置いた障害物が遮るかどうかを示す断面図。" className="mt-2 w-full">
          ...
          <line x1="20" y1={GROUND_Y} x2="560" y2={GROUND_Y} stroke={diagramPalette.line} strokeWidth={diagramStroke.main} />

          {/* 第1フレネルゾーン */}
          <path d={zonePath} fill="rgba(0,113,189,0.12)" stroke={diagramPalette.staf} strokeWidth={diagramStroke.main} />
          {/* 60%クリアランス境界（この線より下に障害物が無ければ実務上クリア） */}
          <path d={clr60Path} fill="none" stroke={diagramPalette.successDeep} strokeWidth={diagramStroke.main} strokeDasharray="5 4" />

          {/* 見通し線(LOS) */}
          <line x1={x(0)} y1={y(txHeightM)} x2={x(1)} y2={y(rxHeightM)} stroke={diagramPalette.staf} strokeWidth="1.8" strokeDasharray="6 4" />
          ...
          {[
            ...
            <g key={ant.label}>
              <line x1={ant.ax} y1={GROUND_Y} x2={ant.ax} y2={y(ant.h)} stroke={diagramPalette.inkSoft} strokeWidth={diagramStroke.emphasis} />
              <circle cx={ant.ax} cy={y(ant.h)} r="4" fill={diagramPalette.staf} />
