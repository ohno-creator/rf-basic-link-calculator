...
const introTermSeeds: IntroTermSeed[] = [
  term("rf", "RF", "用語メモ：RFはRadio Frequencyの略で、無線の入口になる言葉です。RFに最も近い説明は？", "電波・高周波を扱う技術分野", "低周波の音だけを扱う分野", "電池容量だけを扱う分野", "画像圧縮だけを扱う分野", "RFは『電波を使って、ケーブルなしで情報を飛ばす技術』全体をまとめて呼ぶ言葉です。スマホ、Wi-Fi、Bluetooth、テレビのリモコン、ICカードも、みんなRFの仲間です。", "『RF』と聞くと難しそうですが、要は『目に見えない電波で通信する話』です。この冒険では、その電波が“どれくらい強いか”“どこまで届くか”を、用語をひとつずつ覚えながら身につけていきます。"),
  term("radio-wave", "電波", "用語メモ：電波は空間を進む電磁波の一種です。無線通信で使う電波の説明として近いものは？", "電界と磁界が空間を伝わる波", "空気そのものが流れる風", "ケーブル内だけに閉じ込める直流", "音の振動だけで伝わる波", "電波は、目には見えませんが空間を光と同じ速さで進む『波』です。アンテナから出て空気の中を伝わり、相手のアンテナに届きます。水面に石を落とすと輪が広がるのと同じで、遠くへ行くほど弱まります。", "『電波が弱い』ときの原因はひとつではありません。遠すぎる／間に壁がある／アンテナの向きが悪い／まわりがうるさい（ノイズ）――この冒険では、その“弱くなる理由”をひとつずつ見つけていきます。"),
  term("frequency", "周波数", "用語メモ：周波数は1秒間に波が何回振動するかを表します。周波数の単位は？", "Hz", "dBm", "mAh", "Ohm/m", "周波数は『電波が1秒間に何回ふるえるか』を表す数で、単位はHz（ヘルツ）です。たとえば920MHzは1秒に約9.2億回。ラジオやテレビのチャンネルのように、無線は周波数で“場所”を分け合っています。", "周波数が高いほどたくさん情報を運べますが、遠くへは届きにくくなりがちです。低い周波数は遠くまで届きやすい代わりにアンテナが大きくなりがち。どちらが良いかは用途しだいです。", toolLinks.wavelength),
  term("wavelength", "波長", "用語メモ：波長は波の山から次の山までの長さです。周波数が高くなると波長はどうなりますか？", "短くなる", "長くなる", "必ず1mになる", "送信電力と同じになる", "波長は『電波の波ひとつ分の長さ』です。速くふるえる（周波数が高い）ほど、波ひとつは短くなります。アンテナの大きさはこの波長で決まるので、周波数が高いほどアンテナを小さくできます。", "アンテナの話で『1/4波長』『半波長』がよく出るのは、波長に合わせた長さがいちばん電波を出しやすいからです。楽器の弦の長さで音の高さが決まるのと、少し似ています。", toolLinks.wavelength),
  term("mhz", "MHz", "用語メモ：MHzは周波数の単位で、無線仕様書に頻出します。1MHzは何Hzですか？", "1,000,000Hz", "1,000Hz", "1Hz", "1,000,000,000Hz", "MHz（メガヘルツ）の『メガ』は100万のこと。1MHz＝1,000,000Hzで、920MHzは920,000,000（約9.2億）Hzです。無線の仕様書でいちばんよく出てくる単位です。", "桁を1つ間違えるだけで、波長も損失も大きくズレます。まずは『M＝100万、G＝10億』とMHz・GHzの換算に慣れると、計算でつまずきにくくなります。", toolLinks.wavelength),
  term("ghz", "GHz", "用語メモ：GHzはWi-Fiや5Gの話でよく出る周波数単位です。1GHzは何MHzですか？", "1000MHz", "100MHz", "10MHz", "1MHz", "GHz（ギガヘルツ）の『ギガ』は10億のこと。1GHz＝1000MHzです。Wi-Fiの2.4GHzは2400MHz、と読みかえられます。", "比べたいときは、まず単位をそろえるのがコツです。2.4GHzと920MHzを比べるなら、2400MHzと920MHzにそろえる――それだけで損失や波長の大小がぐっと分かりやすくなります。", toolLinks.wavelength),
  term("db", "dB", "用語メモ：dBは倍率を足し算で扱うための単位です。dBの正しい説明は？", "比率を対数で表す単位", "絶対電力を必ず表す単位", "アンテナの長さ", "周波数だけの単位", "dB（デシベル）は『何倍か』を、かけ算ではなく足し算で表す便利な単位です。+3dBで約2倍、+10dBで10倍、−3dBで約半分。これさえ覚えれば、強さの増減を足し引きで追えます。", "なぜわざわざdB？――電波の強さは『100万倍』のような極端な差がふつうに出ます。dBなら『+60dB』のように短く書け、しかも利得（プラス）も損失（マイナス）も全部足し算でまとめられるからです。", toolLinks.dbFeel),
  term("dbm", "dBm", "用語メモ：dBmは1mWを基準にした絶対電力です。0dBmは？", "1mW", "0mW", "1W", "10W", "dBmは『電波の強さそのもの』を表す単位で、1mW（ミリワット）を基準（0dBm）にします。0dBm＝1mW、+30dBm＝1W。dBが“何倍（差）”なのに対し、dBmは“何ワットぶん（絶対値）”です。", "dBとdBmは1文字違いで混ざりやすいです。『dB＝倍率（差）』『dBm＝電力そのもの』。〇dBm から 〇dBm を引くと差（dB）になる、と覚えると整理できます。", toolLinks.dbm),
  term("dbi", "dBi", "用語メモ：dBiは等方性アンテナを基準にしたアンテナ利得です。dBiが表すものは？", "等方性アンテナ基準の利得", "1mW基準の電力", "受信感度の単位", "ケーブル長の単位", "dBiは『アンテナがどれだけ電波を一方向に集められるか（利得）』の単位です。全方向へ均一に出す理想アンテナ（等方性）を0dBiとして、それより何dB強いかを表します。", "懐中電灯をイメージしてください。同じ電池でも、反射鏡で光を一方向に集めれば、その向きはずっと明るくなります。アンテナの利得も同じで、電力を増やすのではなく“向きを集中させて”強く見せています。", toolLinks.wavelength),
  ...
  term("gain", "利得", "用語メモ：利得は電波を特定方向へ集中する性質を表します。アンテナ利得の理解として正しいものは？", "方向による強さの偏りを含む指標", "電源なしで総電力を増やす魔法", "受信感度を消す値", "周波数を低くする値", "利得（ゲイン）は『電波をある方向へどれだけ強く出せるか』です。電源で増幅するのではなく、全方向にばらまく代わりに“狙った方向に集める”ことで、その方向だけ強くします。", "利得が高いアンテナは遠くまで届きやすい反面、向きがずれると一気に弱くなります。強い懐中電灯ほど、少し狙いを外すと真っ暗――距離だけでなく“向き合わせ”も設計のうちです。"),
  term("loss", "損失", "用語メモ：損失は電力が弱くなる要因をdBで表します。リンクバジェットでは損失をどう扱いますか？", "受信電力を下げる方向に引く", "必ず受信電力へ足す", "周波数へ掛ける", "距離の単位に変える", "損失は『電波が弱くなる量』をdBで表したものです。ケーブルを通る・壁を抜ける・遠くまで飛ぶ――どれも電波を弱めます。計算では受信側の強さから“引き算”して扱います。", "損失を“原因べつ”に分けると、対策も分かれます。ケーブルなら短く太く、壁なら置き場所を変える、距離ならアンテナを高く――『どこで何dB失っているか』が見えると、打ち手が決まります。", toolLinks.calculator),
  term("eirp", "EIRP", "用語メモ：EIRPは送信電力、送信側損失、アンテナ利得を合わせた等価放射電力です。EIRPで見るものは？", "アンテナの最も強い方向に出る実効的な放射の強さ", "受信機の雑音温度だけ", "ケーブルの外径", "電池残量", "EIRPは『アンテナがいちばん強く電波を出す向き（主ビーム方向）に、実際どれだけ強く放射しているか』を表します。“もし全方向へ均一に出す理想アンテナ（等方性）に置きかえたら何ワット相当か”という見方で、式では 送信電力 − 給電損失 ＋ アンテナ利得（最大方向）です。", "電波の出力規制は、この EIRP（最大方向の実効的な強さ）で上限が決まることが多いです。利得の高いアンテナに替えるほど、その分だけ送信出力を絞る必要が出てくる――というルールがここから生まれます。", toolLinks.calculator),
  ...
  term("tx-power", "送信電力", "用語メモ：送信電力は送信機が出す電力です。距離を伸ばすために送信電力を上げるとき必要な確認は？", "法規制と機器仕様", "問題文の長さ", "アンテナの色", "HTMLの構造", "送信電力は『送信機が出す電波の強さ』です。大きいほど遠くへ届きやすいですが、無線の種類ごとに法律で上限が決まっていて、自由に上げられるわけではありません。", "送信電力を+3dB（約2倍）にすれば届きやすくなりますが、電池の減り・発熱・まわりへの干渉も増えます。『出力を上げる』は最後の手段で、まずはアンテナや設置で稼ぐのが基本です。", toolLinks.calculator),
  term("rx-power", "受信電力", "用語メモ：受信電力は受信機に届いた信号の強さです。リンク判定で比較する相手は？", "受信感度", "ページ幅", "基板色", "会社名", "受信電力は『相手のアンテナに実際に届いた電波の強さ』です。これが受信機の“聞き取れる限界（受信感度）”より強ければ、通信できる可能性があります。", "受信電力が強くても、まわりのノイズや他の電波（干渉）が強いと聞き取りにくくなります。“声の大きさ”だけでなく“まわりのうるささ”も、通信の良し悪しを決めます。", toolLinks.calculator),
  term("sensitivity", "受信感度", "用語メモ：受信感度は受信機が復調できる最小付近の信号レベルです。値としてはどちらが高感度に近いですか？", "-120dBm", "-90dBm", "+10dBm", "0dB", "受信感度は『受信機が聞き取れる、いちばん小さな電波の強さ』です。−120dBmのように、数字が小さい（マイナスが大きい）ほど、より弱い電波まで聞き取れる＝高感度です。", "受信感度は通信スピードや設定で変わります。ゆっくり送る（低速）ほど弱い電波でも聞き取りやすく、速く送るほど聞き取りにくくなります。仕様書の“どんな条件での感度か”を確認しましょう。", toolLinks.calculator),
  term("link-budget", "リンクバジェット", "用語メモ：リンクバジェットは送信から受信までの利得・損失を足し引きする表です。目的は？", "受信電力と余裕を見積もる", "アンテナを塗装する", "周波数を法律にする", "PDFを圧縮する", "リンクバジェットは『送信から受信まで、電波の強さがどう増えて・減るか』を足し算・引き算でまとめた表です。最後に“受信電力が受信感度を上回るか”で、届くかどうかを判断します。", "家計簿に似ています。収入（送信電力・アンテナ利得）から支出（距離や壁などの損失）を引いて、手元にいくら残るか（受信電力）を見る。残りが必要額（受信感度）を超えていればOK、という考え方です。", toolLinks.calculator),
  term("link-margin", "リンクマージン", "用語メモ：リンクマージンは受信電力が受信感度をどれだけ上回るかです。0dB未満なら？", "通信困難になりやすい", "必ず安定", "距離がゼロ", "アンテナ利得が無限", "リンクマージンは『受信電力が受信感度をどれだけ上回っているか（dB）』、つまり通信の“余裕”です。0dBより大きければ理屈の上では届き、大きいほど環境の変化に強くなります。", "余裕が3dBしかない設計は、人や車が通る・雨が降る・端末の向きが変わる、だけで簡単に切れます。だから現場では10〜20dBの余裕を持たせます。マージンは“安心料”です。", toolLinks.calculator),
  ...
  term("path-loss", "伝搬損失", "用語メモ：伝搬損失は空間を進む間に信号が弱くなる量です。距離が伸びると一般に？", "大きくなる", "必ず小さくなる", "ゼロになる", "受信感度に変わる", "伝搬損失は『電波が空間を進むうちに弱くなる量』です。遠いほど、また周波数が高いほど大きくなり、建物・地面・木などでさらに増えます。", "『何km届く？』という問いは、『その距離の伝搬損失が、リンクバジェットの“余裕”に収まるか？』に言いかえられます。距離の話は、じつは損失の話なのです。", toolLinks.propagation),
  term("fspl", "自由空間損失", "用語メモ：自由空間損失は遮蔽や反射がない理想的な見通しの基準損失です。FSPLの使い方は？", "見通し条件の基準線として使う", "屋内遮蔽を完全再現する", "人体損失を自動計算する", "筐体損失を消す", "自由空間損失（FSPL）は『さえぎる物が何もない理想の見通し』での、距離による弱まり方です。実際はこれに、壁・地面・木などの損失が“足し算”されます。いちばん良いとき（下限）の基準線です。", "FSPLで『届くはず』なのに現場で届かないときは、地面の反射・壁・筐体・人の体・アンテナの向き――FSPLに入っていない“現実の損失”を疑います。FSPLはゴールではなくスタート地点です。", toolLinks.fspl),
  ...
  term("antenna", "アンテナ", "用語メモ：アンテナは電気信号と電波を変換する部品です。無線距離でアンテナが重要な理由は？", "放射効率と方向性に効く", "HTMLを生成する", "電池を充電する", "周波数を法律にする", "アンテナは『電気の信号を電波に変えて空中に出す／空中の電波を電気の信号に戻す』部品です。送る側でも受ける側でも、通信の届きやすさを大きく左右します。", "同じ無線モジュールでも、アンテナと、それを取り付ける筐体や基板しだいで通信距離が何倍も変わることがあります。“最後の数cm”であるアンテナは、地味ですが主役級です。", toolLinks.wavelength),
  ...
  term("impedance", "インピーダンス", "用語メモ：インピーダンスは交流で見た電圧と電流の関係です。RFでよく見る基準値は？", "50Ω", "5V", "1000mAh", "920MHz", "インピーダンスは『電気の流れにくさ（交流での抵抗のようなもの）』です。RFの世界では、多くの機器・ケーブル・アンテナが50Ω（オーム）という共通の値にそろえられています。", "50Ωは魔法の数字ではなく、みんなで決めた“待ち合わせ場所”のようなものです。機器・ケーブル・アンテナを同じ50Ωにそろえると、電波がムダな反射なくスムーズに渡せます。"),
  term("matching", "整合", "用語メモ：整合は送信機・ケーブル・アンテナのインピーダンスを合わせることです。目的は？", "反射を減らして電力を渡しやすくする", "周波数を消す", "距離を固定する", "筐体損失を測る", "整合（マッチング）は『送信機・ケーブル・アンテナの“流れにくさ（50Ω）”をそろえる』ことです。そろっていないと電波の一部が跳ね返って（反射して）戻り、ムダになります。", "ホースの太さが急に変わると水がはねるように、50Ωがそろっていないと電波が跳ね返ります。ただし“整合が良い＝よく飛ぶ”ではありません。きれいに渡せても、飛ばす役（アンテナ）が下手なら届きません。"),
  term("swr", "SWR", "用語メモ：SWRは定在波比で、整合の悪さを示す代表指標です。SWRだけで分からないものは？", "放射効率", "反射の目安", "給電線上の定在波", "送信機から見た負荷", "SWR（定在波比）は『整合がどれくらい悪いか＝どれくらい反射しているか』を表す数字です。1.0が理想で、大きいほど反射が多く、電波がうまく渡せていない状態です。", "SWRが良くても“よく飛ぶ”とは限りません。SWRは『電波をアンテナまでうまく渡せているか』の指標で、その先“どれだけ空へ放射できるか（効率・向き）”は別の話。両方そろって初めて遠くへ届きます。"),
  ...
  term("snr", "SNR", "用語メモ：SNRはSignal-to-Noise Ratio、信号対雑音比です。何を見る指標ですか？", "信号がノイズよりどれだけ強いか", "アンテナの長さ", "ケーブルの太さ", "電池容量", "SNR（信号対雑音比）は『ほしい電波が、まわりのノイズよりどれだけ強いか』です。大きいほど聞き取りやすく、通信が安定します。", "うるさい部屋では大声でも聞き取りにくいですよね。電波も同じで、強さ（RSSI）だけでなく“まわりの静かさ”が効きます。同じ強さでも、ノイズが少ないほうがずっとよく通信できます。"),
  ...
  term("rssi", "RSSI", "用語メモ：RSSIはReceived Signal Strength Indicatorです。主に何を示しますか？", "受信信号強度の目安", "復調成功率そのもの", "アンテナ長", "送信電力の法規値", "RSSIは『受信した電波の強さの目安』を示す値です（スマホの電波バーのもと）。多くの機器が手軽に表示してくれますが、測り方は機器・方式でまちまちな点に注意します。", "RSSI（電波バー）が強くても、まわりがうるさい（ノイズや干渉が多い）と通信は不安定になります。バーの本数だけでなく、“どれだけノイズより目立つか（SNR）”も合わせて見ると、現実に近づきます。"),
  ...
  term("radiated-test", "放射試験", "用語メモ：放射試験は、製品を電波として動かし空中で測る試験で、いわゆるOTA（Over-The-Air）測定にあたります。分かるものは？", "アンテナ・筐体を含む実機の放射／受信性能", "ケーブル単体の抵抗だけ", "会社名", "PDFサイズ", "放射試験は、製品を実際に電波として動かし、空中（Over-The-Air＝OTA）で性能を測る試験です。ケーブルを直結して回路だけを測る『伝導試験』と対になり、アンテナ・筐体・基板を含めた“実機まるごと”の放射／受信性能（放射パターン、TRP、TIS、EIRP など）を、電波暗室などで確認します。", "つまり放射試験は、OTA評価の中心となる“空中での測定”そのものです。ケーブル直結（伝導試験）では良くても、アンテナを筐体に入れて空中で測ると性能が落ちることはよくあります。量産では、実装ばらつき込みの放射性能を空中で確かめないと、現場の通信品質につながりません。"),
  term("trp", "TRP", "用語メモ：TRPはTotal Radiated Power、全放射電力です。何を表しますか？", "全方向へ放射された電力の合計", "最大方向だけの利得", "受信感度", "PDF余白", "TRPは送信機とアンテナを含めた総放射性能を見る指標です。", "EIRPが最大方向（主ビーム方向）の実効的な強さなら、TRPは全方向に放射した電力をぜんぶ足し合わせた“総量”を見る指標です。"),
