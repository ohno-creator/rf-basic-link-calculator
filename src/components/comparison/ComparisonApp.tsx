...
export default function ComparisonApp() {
  ...
  function reset(example: boolean) {
    requestConfirmation({
      title: '入力内容を置き換えますか？',
      description: `現在の両方式の編集内容を${example ? '説明例' : '空の自分の条件'}に置き換えます。`,
      confirmLabel: '置き換える',
      action: () => { setD(decomposed(example)); setM(measured(example)); setErrors({}); setDrafts({}); setGeneration(g => g + 1); setOutput(''); setOld(null); setSavedAt(''); setPreferences(initialPreferences); }
    });
  }
  ...
  function load(raw: string, afterRestore?: () => void) {
    try {
      ...
      requestConfirmation({ title: '保存した入力を復元しますか？', description: '選択中の編集内容を、保存した入力で置き換えます。', confirmLabel: '復元する', action: () => { restore(result.saved); afterRestore?.(); setMessage(`保存時点：${result.saved.savedAt}。現在モデルで再評価しました。${result.warning}`); } });
  ...
  function store() {
    ...
    requestConfirmation({ title: 'このブラウザーに保存しますか？', description: 'この端末・ブラウザーに1件保存し、以前の1件を上書きします。共有端末では機密情報を保存しないでください。', confirmLabel: '保存する', action: () => { try { localStorage.setItem(STORAGE_KEY, JSON.stringify(saveCase(request, preferences))); setMessage('このブラウザーに1件保存しました'); } catch { setMessage('ブラウザーに保存できません。JSON書き出し、または相談用まとめの手動コピーをご利用ください'); } } });
  }
  function removeSaved() {
    requestConfirmation({ title: '保存済みの1件を削除しますか？', description: 'このアプリがブラウザーに保存した1件だけを削除します。現在の入力は残ります。', confirmLabel: '削除する', tone: 'danger', action: () => { try { localStorage.removeItem(STORAGE_KEY); setMessage('保存済み1件を削除しました'); } catch { setMessage('保存データを削除できませんでした'); } } });
  }
  ...
  function closeConfirmation() {
    ...
    window.requestAnimationFrame(() => confirmationReturnFocus.current?.focus());
  }
  function confirmPending() { const action = confirmation?.action; closeConfirmation(); action?.(); }
  function exportJson() {
  ...
  function importJsonText() {
    if (!jsonText.trim()) { setMessage('JSON内容を貼り付けてください。現在の入力は保持しました'); return; }
    if (new Blob([jsonText]).size > MAX_BYTES) { setMessage('JSONは1MB以下にしてください。現在入力は保持しました'); return; }
  ...
  return <main className="comparison-app" id="comparison-main">
    ...
    <div className="no-print">
      ...
      <section className="result" id="comparison-result" tabIndex={-1} aria-labelledby="result-title"><p className="eyebrow">2 · 今回分かること</p>
        ...
        <div className="actions"><a className="primary" href="#next-check">{vm.nextAction}</a><a href="#comparison-input">条件を見直す</a></div>
      ...
      <section className="output" id="comparison-save" tabIndex={-1}><h2>3 · 保存・説明</h2><p>比較入力・メモをアプリから送信しません。保存はこの端末・ブラウザーに残ります。共有端末では機密情報を保存しないでください。</p>
        <div className="actions"><button className="primary" onClick={makeOutput}>相談用まとめを作る</button><button onClick={store}>このブラウザーに1件保存</button><button onClick={() => { try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) load(raw); else setMessage('保存データがありません'); } catch { setMessage('ブラウザー保存を読み込めません。JSONをお使いください'); } }}>保存した1件を復元</button><button onClick={removeSaved}>保存した1件を削除</button><button disabled={evaluation.status === 'invalid'} onClick={exportJson}>別ファイルへJSON書き出し</button></div>
        ...
        <details className="json-paste"><summary>ファイルを選べない場合：JSON内容を貼り付ける</summary>
          <label htmlFor="comparison-json-text">JSON内容を貼り付け<textarea id="comparison-json-text" value={jsonText} spellCheck={false} onChange={e => setJsonText(e.target.value)} /></label>
          <p className="hint">書き出したJSONを検証してから、画面内の確認を経て入力を置き換えます。内容は外部送信しません。</p>
          <div className="actions"><button disabled={!jsonText.trim()} onClick={importJsonText}>貼り付けたJSONを読み込む</button><button disabled={!jsonText} onClick={() => setJsonText('')}>貼り付け内容を消す</button></div>
        </details>
        <p role="status">{message}</p>{savedAt && <p>復元元の保存日時：{savedAt}／画面は現在の入力から再評価</p>}
        {old && <aside><p>旧モデル {old.modelVersion}／保存日時 {old.savedAt}。旧記録は今回の数値結論に採用していません。</p><details><summary>旧記録の入力・保存時点の結果（未検証）</summary><pre>{JSON.stringify(old.request, null, 2)}</pre><pre>{oldSnapshot}</pre></details><button onClick={() => requestConfirmation({ title: '旧記録から再計算しますか？', description: '旧記録の入力から現在モデルで別の比較を作ります。現在の編集内容を置き換えます。', confirmLabel: '再計算する', action: () => { restore(old); setMessage('旧記録から現在モデルで再計算しました。元ファイルは変更していません'); } })}>入力から現在モデルで再計算</button></aside>}
      </section>
    </div>
    {output && <section className="report-preview"><div className="no-print"><h2>相談・印刷用まとめの内容確認</h2><p>下の枠内が印刷対象です。内容を確認して印刷するか、コピーして必要に応じて既存窓口へ貼り付けてください。自動送信・自動入力は行いません。</p><div className="actions"><button onClick={copy}>内容をコピー</button><button onClick={() => window.print()}>比較シートを印刷</button><a href={CONTACT_URL} target="_blank" rel="noopener noreferrer">既存問い合わせ窓口を開く</a><a href="https://www.staf.co.jp/faq.html" target="_blank" rel="noopener noreferrer">サービス・認証の確認先（FAQ）</a></div></div><div className="print-mark">スタッフ株式会社｜comparison-v2｜{vm.basisLabel}</div><pre className="report-text" tabIndex={0}>{output}</pre></section>}
    {confirmation && <div className="confirm-backdrop no-print" onKeyDown={event => {
    ...
    }}><section className="confirm-dialog" role="alertdialog" aria-modal="true" aria-labelledby="comparison-confirm-title" aria-describedby="comparison-confirm-description">
      <h2 id="comparison-confirm-title">{confirmation.title}</h2>
      <p id="comparison-confirm-description">{confirmation.description}</p>
      <div className="actions confirm-actions"><button autoFocus onClick={closeConfirmation}>キャンセル</button><button className={confirmation.tone === 'danger' ? 'danger' : 'primary'} onClick={confirmPending}>{confirmation.confirmLabel}</button></div>
    </section></div>}
  </main>;
}
