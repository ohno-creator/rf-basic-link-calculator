...
奥村-秦/COST 231-Hataで用いる空中線地上高は固定値ではありません。リンクバジェット診断では、送信側アンテナ高を基地局高 `hb`、受信側アンテナ高を移動局高 `hm` として入力値から計算します。画面上でもHata系モデルを選択した場合に、現在の `hb` / `hm` と一般的な適用目安（基地局高30〜200m、移動局高1〜10m、距離1〜20km）を明示します。
